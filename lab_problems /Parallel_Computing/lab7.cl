...
if(row==0 || col==0 ||row==rsize-1 || col==csize-1){
B[row*csize+col]=n;
}
else{
int a[20];
...
for(j=i-1;j>=0;j--){
prod=prod*10+(a[j]?0:1);
}
B[row*csize+col]=prod;
}
}

3)__kernel void mat(__global int *A, __global int *B)
{
int row = get_global_id(0);
int col=get_global_id(1);
int rsize=get_global_size(0);
...
B[col*rsize+row]=A[row*csize+col];;
...
LAB8
q3) __kernel void countOccurences(__global char *A,__global char *B, __global int *lenCount, __global int *count, int srch_len)
{
